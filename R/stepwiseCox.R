...
stepwiseCox <- function(formula,
  ...
  intercept <- getIntercept(formula, data, type = "cox") # char type
  merged_include <- getMergedInclude(include)
  model_raw <- getModel(data, type = "cox", x_name_orig, y_name, weights, intercept)
  ...
  if (strategy=="subset"){ #subset
    ...
    out_init_stepwise <- getInitialStepwise(data,type="cox",strategy,metric,weights,x_name,y_name,intercept,include,method=test_method_cox)
    ...
    out_final_stepwise <- getFinalStepModel(add_or_remove,data,type="cox",metric,weights,y_name,x_in_model,x_notin_model,intercept, include, process_table, test_method_cox,test_method_linear,test_method_logit)
  ...
  table5_coef_model <- getTBLCoefModel(type="cox",intercept,include,x_final_model,y_name,n_y,data,test_method_cox)
