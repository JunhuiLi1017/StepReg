...
In this section, we'll demonstrate how to perform linear regression analysis using the [`mtcars`](https://cran.r-project.org/web/packages/explore/vignettes/explore_mtcars.html) dataset, showcasing different scenarios with varying numbers of predictors and dependent variables. We set `type = "linear"` to direct the function to perform linear regression. 
...
We have developed an interactive Shiny application to simplify model selection tasks for non-programmers. You can access the app through the following URLs:

1. Rstudio url:
2. MCCB url:

Alternatively, you can also access the Shiny app directly from within RStudio:
```{r}
library(StepReg)
# Code to launch the shiny app
