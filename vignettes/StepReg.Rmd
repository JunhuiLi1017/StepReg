...
For details on each criterion, see below:
...
Formula_Linear_univariate <- c("$n\\ln\\left(\\frac{SSE}{n}\\right) + 2p + n + 2$ [@Hurvich_Tsai_1989]",
                              "$n \\ln\\left(\\frac{SSE}{n}\\right) + \\frac{n(n+p)}{n-p-2}$ [@Hurvich_Tsai_1989]",
                              "$n \\ln\\left(\\frac{SSE}{n}\\right) + 2(p+2)o - 2o^2, \\quad o = \\frac{n\\sigma^2}{SSE}$ [@Sawa_1978; @Judge_1985]",
                              "$\\frac{SSE}{\\sigma^2} + 2p - n$ [@Mallows_1973; @Hocking_1976]",
                              "$n \\ln\\left(\\frac{SSE}{n}\\right) + 2p \\ln(\\ln(n))$ [@Hannan_Quinn_1979]",
                              "$n \\ln\\left(\\frac{SSE}{n}\\right) + \\frac{2pn \\ln(\\ln(n))}{n-p-2}$ [@McQuarrie_Tsai_1998]",
                              "$n \\ln\\left(\\frac{SSE}{n}\\right) + p$ [@Nelder_Wedderburn_1972; @Smith_Spiegelhalter_1980]",
                              "$n \\ln\\left(\\frac{SSE}{n}\\right) + \\frac{3}{2}p$ [@Smith_Spiegelhalter_1980]",
                              "$\\textit{F test}$",
                              "$n \\ln\\left(\\frac{SSE}{n}\\right) + p \\ln(n)$ [@Schwarz_1978; @Judge_1985]",
                              "$1 - \\frac{SSE}{SST}$",
                              "$1 - \\frac{(n-1)(1-R^2)}{n-p}$ [@Darlington_1968; @Judge_1985]")

Formula_Linear_multivariate <- c("$n\\ln(|\\text{SSE}_{\\text{det}}|) + 2pq + q(q+1)$ [@Hurvich_Tsai_1989; @Al-Subaihi_2002]",
                              "$n\\ln(|\\text{SSE}_{\\text{det}}|) + \\frac{nq(n+p)}{n-p-q-1}$ [@Hurvich_Tsai_1989; @Bedrick_Tsai_1994]",
                              "",
                              "[@Sparks_1985]",
                              "$n\\ln(|\\text{SSE}_{\\text{det}}|) + 2pq\\ln(\\ln(n))$ [@Hannan_Quinn_1979; @McQuarrie_Tsai_1998; @Hurvich_Tsai_1989]",
                              "$n\\ln(|\\text{SSE}_{\\text{det}}|) + \\frac{2pqnln(\\ln(n))}{n-p-q-1}$ [@McQuarrie_Tsai_1998; @Hurvich_Tsai_1989]",
                              "$n\\ln(|\\text{SSE}_{\\text{det}}|) + pq$ [@Nelder_Wedderburn_1972; @Smith_Spiegelhalter_1980]",
                              "$n\\ln(|\\text{SSE}_{\\text{det}}|) + \\frac{3}{2}pq$ [@Smith_Spiegelhalter_1980]",
                              "$\\textit{Approximate F test}$",
                              "$n\\ln(|\\text{SSE}_{\\text{det}}|) + p \\ln(n)$ [@Hurvich_Tsai_1989; @Schwarz_1978; @Judge_1985; @Al-Subaihi_2002]",
                              "to be added",
                              "$1 - \\frac{(n-1)(1-R^2)}{n-p}$ [@Darlington_1968; @Judge_1985]")
Formula_Logit_Cox_Poisson_Gamma <- c("$-2{LL} + 2p$  [@Darlington_1968; @Judge_1985]",
                              "$-2{LL} + \\frac{n(n+p)}{n-p-2}$ [@Hurvich_Tsai_1989]",
                              "",
                              "",
                              "$-2{LL} + 2p\\ln(\\ln(n))$",
                              "$-2{LL} + \\frac{2pn \\ln(\\ln(n))}{n-p-2}$",
                              "$-2{LL} + p$ [@Nelder_Wedderburn_1972; @Smith_Spiegelhalter_1980]",
                              "$-2{LL} + \\frac{3}{2}p$ [@Smith_Spiegelhalter_1980]",
                              "logistic,Poisson, and Gamma: score and LRT test for forward, wald test for backward; Cox: LRT test for forward, wald test for backward",
                              "$-2{LL} + p\\ln(n)$",
                              "not available",
                              "not available")
...
There is some inconsistency in the literature on the precise definitions for the AIC and AICC statistics. In the context of linear regression, several different versions of the formulas for AIC and AICC appear in the statistics literature. The following formulas for AIC [@Darlington_1968; @Judge_1985] and AICC [@McQuarrie_Tsai_1998]:

AIC=$n\ln\left(\frac{SSE}{n}\right) + 2p$

AICc=$\ln\left(\frac{SSE}{n}\right) + 1 + \frac{2(p+1)}{n-p-2}$

The definitions of AIC and AICC found in [@Hurvich_Tsai_1989]:

AIC=$n\ln\left(\frac{SSE}{n}\right) + 2p + n + 2$

AICc=$n\ln\left(\frac{SSE}{n}\right) + \frac{n(p+n)}{n-p-2}$

However, for a fixed number of observations, these different versions differ by additive and positive multiplicative constants.
