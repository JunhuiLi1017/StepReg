...
Method <- c("Forward Selection", "Backward Elimination", "Bidirectional Elimination", "Best Subsets")
...
df <- data.frame(Method, Description)
...
Formula_in_Linear <- c("$n\\ln\\left(\\frac{|\\text{SSE}|}{n}\\right) + 2pq + n + q(q+1)$ [@Hurvich_Tsai_1989; @Al-Subaihi_2002]",
                                 "$n\\ln\\left(\\frac{|\\text{SSE}|}{n}\\right) + \\frac{nq(n+p)}{n-p-q-1}$ [@Hurvich_Tsai_1989; @Bedrick_Tsai_1994]",
...
"$n\\ln\\left(\\frac{|\\text{SSE}|}{n}\\right) + 2pq\\ln(\\ln(n))$ [@Hannan_Quinn_1979; @McQuarrie_Tsai_1998; @Hurvich_Tsai_1989]",

"$n\\ln\\left(\\frac{|\\text{SSE}|}{n}\\right) + \\frac{2pqnln(\\ln(n))}{n-p-q-1}$ [@McQuarrie_Tsai_1998; @Hurvich_Tsai_1989] not available in StepReg for MMR",
                              "$n\\ln\\left(\\frac{|\\text{SSE}|}{n}\\right) + p$ [@Nelder_Wedderburn_1972; @Smith_Spiegelhalter_1980] not available in StepReg for MMR",
                              "$n\\ln\\left(\\frac{|\\text{SSE}|}{n}\\right) + \\frac{3}{2}p$ [@Smith_Spiegelhalter_1980] not available in StepReg for MMR",

"$\\textit{F test}$ for UMR and $\\textit{Approximate F test}$ for MMR",

"$n\\ln\\left(\\frac{|\\text{SSE}|}{n}\\right) + p \\ln(n)$ [@Hurvich_Tsai_1989; @Schwarz_1978; @Judge_1985; @Al-Subaihi_2002] not available in StepReg for MMR",
...
Formula_in_Logit_Cox_Poisson_Gamma <- c("$-2\\text{LL} + 2p$  [@Darlington_1968; @Judge_1985]",
                              "$-2\\text{LL} + \\frac{n(n+p)}{n-p-2}$ [@Hurvich_Tsai_1989]",
                              "not available",
                              "not available",
                              "$-2\\text{LL} + 2p\\ln(\\ln(n))$ [@Hannan_Quinn_1979]",
                              "$-2\\text{LL} + \\frac{2pn \\ln(\\ln(n))}{n-p-2}$ [@McQuarrie_Tsai_1998]",
                              "$-2\\text{LL} + p$ [@Nelder_Wedderburn_1972; @Smith_Spiegelhalter_1980]",
                              "$-2\\text{LL} + \\frac{3}{2}p$ [@Smith_Spiegelhalter_1980]",
                              ...
                              "$-2\\text{LL} + p\\ln(n)$ [@Schwarz_1978; @Judge_1985]",
                              "not available",
                              "not available")
df <- data.frame(Abbreviation, Definition, Formula_in_Linear,Formula_in_Logit_Cox_Poisson_Gamma)

kable_styling(footnote(kable(df, format = "html", align='l', booktabs=TRUE, escape = F, caption = 'Abbreviation, Definition, and Formula of the Information Criterion for Linear, Logit, Cox, Possion and Gamma regression'),alphabet = c("In the literature, there is some inconsistency regarding the precise definitions of the AIC and AICC statistics. Within the context of linear regression, multiple variations of the formulas for AIC and AICC are found in statistical literature. 
...
## How to choose variable selection method and information criterion
