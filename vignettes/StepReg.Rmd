...
Various *selection metrics* can be used to guide the process of adding or removing predictors from the model. These metrics help to determine the importance or significance of predictors in improving the model fit. In StepReg, selection metrics include two categories: *Information Criteria* and *Significance Level* of the coefficient associated with each predictor. *Information Criteria* is a means of evaluating a model's performance, which balances model fit with complexity by penalizing models with a higher number of parameters. Lower *Information Criteria* values indicate a better trade-off between model fit and complexity. Note that when comparing different models, it's important to evaluate them within the same *Information Criteria* framework rather than across multiple Information Criteria. This means that if you are using, for example, the AIC to compare models, you should compare all models using AIC. Similarly, if you are using BIC, compare models using BIC only. This ensures consistency and fairness in model comparison, as each Information Criterion has its own scale and penalization factors. Comparing models across different Information Criteria may lead to biased conclusions.

In practice, multiple metrics have been proposed, the ones supported by StepReg are summarized below.
...
In practice, no metric is necessarily optimal for prediction across all datasets. The choice of the metric depends on your specific data and research goals. We recommend using various metrics simultaneously when conducting stepwise regression. This approach enables you to select the most suitable model based on your specific needs. Below summarizes general guidance.
...
+ BIC and SBC: Both BIC and SBC are variants of Bayesian Information Criterion. The precise definition of them differ in literature and in SAS. Here, BIC mirrors the definition of Sawa Bayesion Information Criterion from [SAS](https://documentation.sas.com/doc/en/pgmsascdc/9.4_3.3/statug/statug_glmselect_syntax07.htm) while SBC refers to Schwarz Bayesian Information Criterion. Compared to AIC, the only difference is that Bayesian Information Criterion penalizes the sample size in formula. In practice, if you prefer simpler models or have concerns about overfitting, SBC may be a better choice. SBC may cause the model to under fit [@SAS_Institute_2018; @Sawa_1978; @Hurvich_Tsai_1989; @Schwarz_1978; @Judge_1985; @Al-Subaihi_2002].

+ HQ: An alternative to AIC, but have seen little use in practice. [@Burnham_2002] 

+ Rsq: The R² statistic for model comparison, on the other hand, is proportional to the Log-Likelihood. Therefore, comparing models based on R² is akin to selecting them solely based on their fit to the data, disregarding the model complexity. Higher adjusted R-squared values indicate better model fit.

+ adjRsq: The Adjusted-R² statistic shares similar objectives to information criteria, as both aim to compare models based on their fit and the number of parameters. Higher adjusted R-squared values indicate better model fit. However, information criteria are generally considered superior in this regard [@Stevens_2016].

+ SL: P-values and statistical Significance Levels is an alternative strategy for models comparison. Use p-values and assess statistical significance when you want to determine whether individual predictors have a significant effect on the response variable. If the p-value associated with a predictor's coefficient is below a certain significance level (e.g., 0.05), you would consider the predictor statistically significant, indicating that it has a non-zero effect on the response variable. P-values and statistical Significance Levels are useful for hypothesis testing and inference.

In summary, use p-values and statistical significance when you're interested in the effect of individual predictors, while use information criteria when you're comparing and selecting among different models.
