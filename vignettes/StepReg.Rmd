...
Depending on the number of selected regression strategies and metrics, you can expect to receive at least four tables from `stepwise()`. The following example uses two strategies (`strategy = c("forward", "bidirection")`) and two metrics (`metric = c("AIC", "BIC")`), and ten tables are generated.
```{r, message = FALSE}
data(mtcars)
...
Below describes the content of each table.
```{r, echo = FALSE}
...
You can also save the output into various formats such as "xlsx", "docx", "html", "pptx", and more, for easy sharing. Of note, the suffix will be appended automatically to the `report_name`. The following example generates both "results.xlsx" and "results.docx" reports.
```{r, eval = FALSE}
...
You should select the appropriate regression model according to the response variable type. Refer to section \@ref(regressioncategories) for more information. Below, we provide multiple examples using distinct models with different datasets.
...
> + Visulization of the selection process using `forward` and `bidirection` strategy under information criteron `IC(3/2)` and `AIC`.
