# obtain gold res using v1.4.4 
# output (res_v1_4_4) contains results from all combinations using the following models:
...
for (model in c("linear_model1", "linear_model2", "logit_model1", "cox_model1")) {
    ...
    # 5: perform test
    for (strategy in c("forward", "backward", "bidirection", "score")) {
        if (type == "linear" && strategy == "score") {
        ...
        for (metric in metrics) {
            ...
            res <- NA
            try(res <- test_func(formula = get(model),
                                 data = dataset,
                                 selection = strategy,
                                 select = metric)[[3]], silent = TRUE)
            message(length(res))
            res_v1_4_4[[model]][[strategy]][[metric]] <- res
        }
    }
}

outdir <- system.file("R/tests/data", package = "StepReg")
saveRDS(res_v1_4_4, paste0(outdir, "/res_v1_4_4.rds"))
